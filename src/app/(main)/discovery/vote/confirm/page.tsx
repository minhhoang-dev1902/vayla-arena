...
import { AlertCircle, CheckCircle2, ExternalLink, Music2, RefreshCw } from "lucide-react";
...
/* ── Success overlay ── */
function SuccessOverlay({ txHash, onClose }: { txHash: string; onClose: () => void }) {
	return (
		...
		<div className="fixed inset-0 z-[200] flex flex-col items-center bg-gradient-to-b from-[#fff1f1] via-[#fff8f8] to-white px-6 pt-16 pb-8">
			...
			<div className="mt-auto w-full max-w-sm pt-8">
				...
				<button
					type="button"
					onClick={onClose}
					className="mt-3 flex h-12 w-full items-center justify-center rounded-full border border-[#e2e8f0] bg-white text-sm font-extrabold tracking-wider text-[#64748b] transition hover:bg-slate-50"
				>
					Close
				</button>
			</div>
...
export default function DiscoveryVoteConfirmPage() {
	...
	return (
		<>
				...
				<div className="min-h-0 flex-1 overflow-y-auto overscroll-y-contain px-5 py-8">
					...
					<div className="mt-8 rounded-2xl border border-[#e8f0f6] bg-white p-5">
						<div className="flex items-center gap-2">
							...
							<h2 className="text-base font-bold text-[#0f172a]">Vote Summary</h2>
						</div>

						<div className="mt-4 space-y-3">
							<div className="flex items-center justify-between border-b border-[#f1f5f9] pb-3">
								<span className="text-sm text-[#64748b]">Track</span>
								<div className="text-right">
									<p className="text-sm font-bold text-[#0f172a]">{payload.track.title}</p>
									<p className="text-xs text-[#94a3b8]">{payload.track.artist}</p>
								</div>
							</div>
							<div className="flex items-center justify-between border-b border-[#f1f5f9] pb-3">
								<span className="text-sm text-[#64748b]">Votes</span>
								<span className="text-sm font-bold text-[#0f172a]">{payload.voteCount}</span>
							</div>
							<div className="flex items-center justify-between">
								<span className="text-sm font-bold text-[#0f172a]">Total Cost</span>
								<span className="text-[17px] font-extrabold text-primary">
									{payload.totalCost} VAYLA
								</span>
							</div>
						</div>
					</div>
			...
			{pageState === "success" && (
				<SuccessOverlay txHash={txHash} onClose={() => router.push("/discovery")} />
			)}
